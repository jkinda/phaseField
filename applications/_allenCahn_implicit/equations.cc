...
void variableAttributeLoader::loadVariableAttributes(){
  ...
  set_dependencies_value_term_RHS(0, "n, old(n)");
...
void customPDE<dim,degree>::nonExplicitEquationRHS(variableContainer<dim,degree,dealii::VectorizedArray<double> > & variable_list,
  ...
  //Getting gradient value
  scalargradType nx = variable_list.get_scalar_gradient(0);
  
  scalarvalueType fnV = 4.0*n*n*n - 6.0*n*n + 2.0*n;
  
  scalarvalueType eq_n = (n_old - n -constV(userInputs.dtValue*MnV)*fnV);
  scalargradType eqx_n = (-constV(userInputs.dtValue*KnV*MnV)*nx);
...
void customPDE<dim,degree>::equationLHS(variableContainer<dim,degree,dealii::VectorizedArray<double> > & variable_list,
  ...
  //Getting current value
  scalarvalueType n = variable_list.get_scalar_value(0);
  //Getting change in value
  scalarvalueType Dn = variable_list.get_change_in_scalar_value(0);
  //Getting gradient of change in value
  scalargradType Dnx = variable_list.get_change_in_scalar_gradient(0);
  
  scalarvalueType fnnV = 12.0*n*n - 12.0*n* + 2.0;
  
  scalarvalueType eq_n = Dn*(constV(1.0) + constV(userInputs.dtValue*MnV)*fnnV);
  
  scalargradType eqx_n = constV(KnV*MnV*userInputs.dtValue)*Dnx;
