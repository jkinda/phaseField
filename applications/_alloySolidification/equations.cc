...
void variableAttributeLoader::loadVariableAttributes(){
    ...
    set_dependencies_value_term_RHS(0, "U,mu,phi,grad(phi),grad(U)");
    set_dependencies_gradient_term_RHS(0, "U,grad(U),grad(phi),phi,mu");
...
scalarvalueType d_temp =(constV(104.0)+constV(47.9)*tep);
    
// Mobility freeze factor
scalarvalueType M_phi;
M_phi = (constV(0.5)+constV(0.5)*(std::exp(constV(2.0)*d_temp)-constV(1.0))/(std::exp(constV(2.0)*d_temp)+constV(1.0)));
...
scalarvalueType q_phi = ((constV(1.0)-phi+constV(k*Ds/Dl)*(constV(1.0)+phi))/constV(2.0));
...
// Antitrapping term
scalargradType j_at;
    j_at= constV(1.0/2.0/sqrt(2.0))*(constV(1.0)+constV(1.0-k)*U)*(mu/u_phi)*normal;
    
// grad_phi and grad_U dot product term
scalarvalueType con_term = (constV((1.0-k)/2.0)/u_con/u_con*(phix[0]*(Dtilt*q_phi*Ux[0]+j_at[0])+phix[1]*(Dtilt*q_phi*Ux[1]+j_at[1])));


// Define required equations
scalarvalueType eq_U =  (U+constV(userInputs.dtValue)*M_phi*(constV(1.0)+constV(1.0-k)*U)*mu/constV(2.0)/u_phi/u_con-constV(userInputs.dtValue)*M_phi*con_term);

scalargradType eqx_U =  (constV(-1.0)*constV(userInputs.dtValue)*M_phi*(Dtilt*q_phi*Ux+j_at)/u_con);
    

scalarvalueType eq_phi = (phi+constV(userInputs.dtValue)*mu*M_phi/u_phi);
...
// The anisotropy term that enters in to the  equation for mu
 ...
 scalarvalueType eq_mu =
    ((phi-constV(lamda)*(U+tep+constV(0.9))*(constV(1.0)-phi*phi))*(constV(1.0)-phi*phi));
